...
if st.session_state.get('datos_cargados', False):
    ...
    st.write(f"df_asin: {st.session_state.df_asin.shape}")
    st.write(f"Avoids: {st.session_state.avoids_df.shape}")
    st.write(f"CustKW: {st.session_state.df_kw.shape}")
    st.write(f"CompKW: {st.session_state.df_comp_data.shape}")
    st.write(f"MiningKW: {st.session_state.df_mining_kw.shape}")
    ...
    with st.expander("Tabla Maestra de Datos Compilados", expanded=True):
        ...
        df_mining = st.session_state.df_mining_kw.iloc[:, [0, 2, 5, 12, 15]].copy() if not st.session_state.df_mining_kw.empty else pd.DataFrame()
