...
if st.session_state.get('datos_cargados', False):
    ...
    with st.expander("Datos del cliente", expanded=False):
        ...
        if subtabs == "Listado de ASINs":
            ...
            st.dataframe(df_kw_filtrado[columnas].reset_index(drop=True))
    ...
    avoid_list = pd.concat([avoids_display[col] for col in avoid_column_names]).dropna().unique().tolist()
    ...
    st.dataframe(df_cust_filtered.reset_index(drop=True))
    ...
    st.dataframe(df_comp_filtered.reset_index(drop=True))
