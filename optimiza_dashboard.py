    ...
    with st.expander("Análisis de Volumen de Búsqueda", expanded=True):
        ...
        for metric, name in metric_names.items():
            ...
            if not data_series.empty:
                ...
                col1.metric("Media", f"{data_series.mean():,.2f}")
                col2.metric("Mediana", f"{data_series.median():,.2f}")
                col3.metric("Asimetría", f"{data_series.skew():,.2f}")
                col4.metric("Desv. Estándar", f"{data_series.std():,.2f}")

                col5, col6, col7, _ = st.columns(4)
                col5.metric("P75", f"{data_series.quantile(0.75):,.2f}")
                col6.metric("P40", f"{data_series.quantile(0.40):,.2f}")
                mode_val = data_series.mode()
                display_mode = mode_val.iloc[0] if not mode_val.empty else "N/A"
                col7.metric("Moda", display_mode)
            st.divider()
