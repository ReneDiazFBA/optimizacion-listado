...
def mostrar_histogramas(df):
    ...
    st.subheader("Distribución de Métricas Normalizadas (Z-Scores)")

    metric_map = {
        'Z-Score SV': 'Search Volume',
        'Z-Score CS Cliente': 'Click Share (Cliente)',
        'Z-Score Rank Depth': 'Rank Depth',
        'Z-Score TCS': 'Total Click Share',
        'Z-Score Relevance': 'Relevance'
    }
    
    cols = st.columns(2)
    col_keys = list(metric_map.keys())

    for i, metric_col in enumerate(col_keys):
        with cols[i % 2]:
