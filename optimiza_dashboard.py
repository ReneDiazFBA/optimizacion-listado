...
if st.session_state.get('datos_cargados', False):
    ...
    with st.expander("Análisis de Volumen de Búsqueda", expanded=True):
        ...
        # Iterar para mostrar estadísticas de cada métrica
        metric_names = {
        ...
        for metric, name in metric_names.items():
            ...
            if not data_series.empty:
                ...
                c4.metric("P75", f"{data_series.quantile(0.75):,.2f}")
                c5.metric("P40", f"{data_series.quantile(0.40):,.2f}")
                c6.metric("Desv. Estándar", f"{data_series.std():,.2f}")
                # El modo puede devolver varios valores, mostramos el primero
                c7.metric("Moda", f"{data_series.mode().iloc[0] if not data_series.mode().empty else 'N/A':,.2f}")
            st.divider()
