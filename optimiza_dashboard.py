...
if st.session_state.get('datos_cargados', False):
    ...
    with st.expander("Análisis de Volumen de Búsqueda", expanded=True):
        ...
        # --- INICIO DE LA NUEVA TABLA NORMALIZADA ---
        st.divider()
        st.subheader("Tabla Maestra Normalizada")
        
        df_normalizada = pd.DataFrame()
        df_normalizada['Search Terms'] = result_df['Search Terms']
        
        df_normalizada['Search Volume'] = None
        df_normalizada['Click Share (Cliente)'] = None
        df_normalizada['Rank Depth'] = None
        df_normalizada['Total Click Share'] = None
        df_normalizada['Relevance'] = None
        df_normalizada['FODA'] = None
        
        st.dataframe(df_normalizada.reset_index(drop=True))
        # --- FIN DE LA NUEVA TABLA NORMALIZADA ---
