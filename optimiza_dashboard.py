...
def mostrar_histogramas(df):
    ...
    st.subheader("Distribución de Métricas (Histogramas)")
    ...
    for metric_col, display_name in metric_map.items():
        ...
        if not data_series.empty:
            ...
            ax.set_ylabel('Frecuencia de Keywords')
            st.pyplot(fig)
        else:
...
if st.session_state.get('datos_cargados', False):
    ...
    with st.expander("Análisis de Volumen de Búsqueda", expanded=True):
        ...
        # --- INICIO DE NUEVA SECCIÓN DE GRÁFICAS ---
        mostrar_histogramas(numeric_data_for_stats)
        # --- FIN DE NUEVA SECCIÓN DE GRÁFICAS ---


    with st.expander("Datos para Análisis", expanded=False):
        # (Aquí va todo el código de las secciones de datos detallados)