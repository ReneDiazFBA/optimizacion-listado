...
def inicializar_datos(archivo_subido):
    try:
        ...
        st.session_state.df_kw = cust_kw_raw.iloc[2:, [0, 1, 15, 25]].copy()
        ...
        st.session_state.df_comp_data = comp_kw_raw.iloc[2:, [0, 2, 5, 8, 18]].copy()
        ...
        if 'MiningKW' in xls.sheet_names:
            ...
            st.session_state.df_mining_kw = mining_kw_raw.iloc[2:, [0, 100, 5, 12, 15]].copy()
        ...
        if 'MiningUnique' in xls.sheet_names:
            st.session_state.df_mining_unique = pd.read_excel(archivo_subido, sheet_name="MiningUnique", header=1)
        else:
