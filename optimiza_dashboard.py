...
def inicializar_datos(archivo_subido):
    ...
    try:
        ...
        # --- LÓGICA DE CARGA ROBUSTA (CORREGIDA) ---
        # Se leen las hojas de keywords complejas UNA SOLA VEZ y se procesan
        ...
        # Carga CompKW una vez y guarda tanto el raw (para ASINs) como los datos procesados
        comp_kw_raw = pd.read_excel(archivo_subido, sheet_name="CompKW", header=None)
        st.session_state.comp_kw_raw = comp_kw_raw  # Se guarda el raw completo
    ...
    with st.expander("Datos para Análisis", expanded=False):
        ...
        with st.expander("Datos de competidores", expanded=False):
            ...
            with st.expander("Ver/Ocultar ASINs de Competidores", expanded=True):
                # --- LÓGICA CORREGIDA ---
                # Usar el dataframe 'raw' que se guardó al inicio
