...
if st.session_state.get('datos_cargados', False):
    ...
    with st.expander("Análisis de Volumen de Búsqueda", expanded=True):
        ...
        # Limpiar y calcular columnas numéricas
        ...
        # Usar los datos numéricos de la tabla filtrada para los cálculos
        numeric_data_for_stats = df_filtrado_vol
        
        st.markdown("**Search Volume**")
        col1, col2, col3 = st.columns(3)
        col1.metric("Media", f"{numeric_data_for_stats['Search Volume'].mean():,.2f}")
        col2.metric("Mediana", f"{numeric_data_for_stats['Search Volume'].median():,.0f}")
        col3.metric("Asimetría", f"{numeric_data_for_stats['Search Volume'].skew():,.2f}")
        col4, col5, col6 = st.columns(3)
        col4.metric("P75", f"{numeric_data_for_stats['Search Volume'].quantile(0.75):,.0f}")
        col5.metric("P40", f"{numeric_data_for_stats['Search Volume'].quantile(0.40):,.0f}")
        col6.metric("Desv. Estándar", f"{numeric_data_for_stats['Search Volume'].std():,.2f}")
        st.divider()

        st.markdown("**Click Share (Cliente) (%)**")
        col7, col8, col9 = st.columns(3)
        col7.metric("Media", f"{numeric_data_for_stats['CS_Cliente_Num'].mean():,.2f}")
        col8.metric("Mediana", f"{numeric_data_for_stats['CS_Cliente_Num'].median():,.2f}")
        col9.metric("Asimetría", f"{numeric_data_for_stats['CS_Cliente_Num'].skew():,.2f}")
        col10, col11, col12 = st.columns(3)
        col10.metric("P75", f"{numeric_data_for_stats['CS_Cliente_Num'].quantile(0.75):,.2f}")
        col11.metric("P40", f"{numeric_data_for_stats['CS_Cliente_Num'].quantile(0.40):,.2f}")
        col12.metric("Desv. Estándar", f"{numeric_data_for_stats['CS_Cliente_Num'].std():,.2f}")
        st.divider()
        
        st.markdown("**Rank Depth**")
        col13, col14, col15 = st.columns(3)
        col13.metric("Media", f"{numeric_data_for_stats['Rank_Depth_Num'].mean():,.2f}")
        col14.metric("Mediana", f"{numeric_data_for_stats['Rank_Depth_Num'].median():,.0f}")
        col15.metric("Asimetría", f"{numeric_data_for_stats['Rank_Depth_Num'].skew():,.2f}")
        col16, col17, col18 = st.columns(3)
        col16.metric("P75", f"{numeric_data_for_stats['Rank_Depth_Num'].quantile(0.75):,.0f}")
        col17.metric("P40", f"{numeric_data_for_stats['Rank_Depth_Num'].quantile(0.40):,.0f}")
        col18.metric("Desv. Estándar", f"{numeric_data_for_stats['Rank_Depth_Num'].std():,.2f}")
        st.divider()
        
        st.markdown("**Total Click Share (%)**")
        col19, col20, col21 = st.columns(3)
        col19.metric("Media", f"{numeric_data_for_stats['TCS_Num'].mean():,.2f}")
        col20.metric("Mediana", f"{numeric_data_for_stats['TCS_Num'].median():,.2f}")
        col21.metric("Asimetría", f"{numeric_data_for_stats['TCS_Num'].skew():,.2f}")
        col22, col23, col24 = st.columns(3)
        col22.metric("P75", f"{numeric_data_for_stats['TCS_Num'].quantile(0.75):,.2f}")
        col23.metric("P40", f"{numeric_data_for_stats['TCS_Num'].quantile(0.40):,.2f}")
        col24.metric("Desv. Estándar", f"{numeric_data_for_stats['TCS_Num'].std():,.2f}")
        st.divider()

        st.markdown("**Relevance**")
        col25, col26, col27 = st.columns(3)
        col25.metric("Media", f"{numeric_data_for_stats['Relevance_Num'].mean():,.2f}")
        col26.metric("Mediana", f"{numeric_data_for_stats['Relevance_Num'].median():,.0f}")
        col27.metric("Asimetría", f"{numeric_data_for_stats['Relevance_Num'].skew():,.2f}")
        col28, col29, col30 = st.columns(3)
        col28.metric("P75", f"{numeric_data_for_stats['Relevance_Num'].quantile(0.75):,.0f}")
        col29.metric("P40", f"{numeric_data_for_stats['Relevance_Num'].quantile(0.40):,.0f}")
        col30.metric("Desv. Estándar", f"{numeric_data_for_stats['Relevance_Num'].std():,.2f}")