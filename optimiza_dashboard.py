...
if st.session_state.get('datos_cargados', False):
    with st.expander("Datos para Análisis", expanded=False):
        ...
        with st.expander("Datos del cliente", expanded=False):
            # Aquí va tu bloque de Cliente (Listing ASIN + Reverse ASIN Cliente) como lo tienes
            # Agrega checkbox para "Deshabilitar Filtro" como ya te lo mostré en el ejemplo previo.

        # --- Datos de Competidores ---
        with st.expander("Datos de competidores", expanded=False):
            # Igual aquí tu bloque de Competidores Reverse ASIN, con su checkbox de filtro desactivable.
        ...
        if st.session_state.get('df_mining_kw') is not None and not st.session_state.df_mining_kw.empty:
            with st.expander("Minería de Search Terms", expanded=False):
                # Aquí tu bloque de Minería, añadiendo checkbox "Deshabilitar filtro" para scroll.
        # --- Palabras Únicas ---
        with st.expander("Palabras Únicas", expanded=False):
            # Aquí va tu bloque de palabras únicas con el flujo de avoids como ya tienes.

    # --- Tabla Maestra de Datos Compilados (sin expansores anidados) ---
