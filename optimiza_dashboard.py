...
def inicializar_datos(archivo_subido):
    ...
    try:
        ...
        # Se leen las hojas de keywords
        st.session_state.df_kw = pd.read_excel(archivo_subido, sheet_name="CustKW", header=None, skiprows=2)
        
        # Carga completa de CompKW para ASINs y datos
        df_comp_data_full = pd.read_excel(archivo_subido, sheet_name="CompKW", header=None)
        st.session_state.df_comp_data_full = df_comp_data_full # Guardado para extraer ASINs
        st.session_state.df_comp_data = df_comp_data_full.iloc[2:].copy() # Datos desde fila 3
    ...
    with st.expander("Datos para Análisis", expanded=False):
        ...
        with st.expander("Datos de competidores", expanded=False):
            ...
            with st.expander("Ver/Ocultar ASINs de Competidores", expanded=True):
                # --- LÍNEA CORREGIDA ---
                # Usar el nombre de variable correcto que fue guardado en inicializar_datos
